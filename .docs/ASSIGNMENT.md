...
Create 144 sprites (**NOT** graphic objects) that are stacked on top of each other like cards in a deck. The top card must cover the bottom card, but not completely.
...
Use it to render a dialogue between characters with the data taken from this endpoint:
